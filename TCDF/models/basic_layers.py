...
class Transformer(nn.Module):
    def __init__(
        self, *,
        num_frames, token_len, save_hidden, dim, depth, heads, mlp_dim,
    ...
    def forward(self, x):
        ...
        if self.token_len is not None:
            if n > self.max_frames:
                raise ValueError(
                    f"Input sequence length {n} exceeds configured num_frames {self.max_frames}"
                )
...
class CrossTransformer(nn.Module):
    def __init__(
        self, *,
        source_num_frames, tgt_num_frames, dim, depth, heads, mlp_dim,
        pool='cls', dim_head=64, dropout=0., emb_dropout=0.
    ):
        super().__init__()

        self.source_max_frames = source_num_frames
        self.target_max_frames = tgt_num_frames
    ...
    def forward(self, source_x, target_x):
        ...
        if n_s > self.source_max_frames:
            raise ValueError(
                f"Source sequence length {n_s} exceeds configured source_num_frames {self.source_max_frames}"
            )
        if n_t > self.target_max_frames:
            raise ValueError(
                f"Target sequence length {n_t} exceeds configured tgt_num_frames {self.target_max_frames}"
            )
